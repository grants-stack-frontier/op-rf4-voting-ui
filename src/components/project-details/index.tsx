...
export function ProjectDetails({ data, isPending }: { data?: Project; isPending: boolean }) {
	const { profileAvatarUrl, name, projectCoverImageUrl, description, socialLinks, applicationCategory, organization, github, links, grantsAndFunding, pricingModel, impactStatement, contracts, team } = data ?? {};
	return (
		<>
			{isPending ? (
				<>
				...
				<>
					<ProjectHeader profileAvatarUrl={profileAvatarUrl} name={name} projectCoverImageUrl={projectCoverImageUrl} organization={organization} />
