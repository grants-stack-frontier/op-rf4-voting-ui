  ...
  return (
    <div className="flex flex-wrap items-center gap-4 my-6">
      {(website && (Array.isArray(website) && website?.length > 0)) && (
        <Link className="flex items-center gap-1.5 truncate max-w-[200px] text-sm lowercase hover:underline" href={getSafeUrl(website)} target="_blank" onClick={() => mixpanel.track("Open Website", { external: true })}>
      ...
      {(farcaster && (Array.isArray(farcaster) && farcaster?.length > 0)) && (
        <Link className="flex items-center gap-1.5 hover:underline" href={getSafeUrl(farcaster)} target="_blank" onClick={() => mixpanel.track("Open Farcaster", { external: true })}>
      ...
      {twitter && (
        <Link className="flex items-center gap-1.5 max-w-[200px] truncate text-sm lowercase hover:underline" href={getSafeUrl(twitter)} target="_blank" onClick={() => mixpanel.track("Open Twitter", { external: true })}>
      ...
      {mirror && (
        <Link className="flex items-center gap-1.5 truncate max-w-[200px] text-sm lowercase hover:underline" href={getSafeUrl(mirror)} target="_blank" onClick={() => mixpanel.track("Open Mirror", { external: true })}>
