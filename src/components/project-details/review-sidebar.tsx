		...
		<Card className={cn("w-[304px] h-[560px] sticky top-8", className)} {...props}>
							...
							<Button
								...
								className={cn(
									label === "Conflict of interest" ? "hover:bg-red-200 hover:text-red-600" :
										label !== "Skip" ? "hover:bg-blue-200 hover:text-blue-600" :
											"",
									isVoted && currentProjectScore === Number(score) && label !== "Conflict of interest" ? "bg-green-200 text-green-600" :
										isVoted && currentProjectScore === 0 && label === "Conflict of interest" ? "bg-red-200 text-red-600" :
											"",
								)}
			...
			<CardFooter className="flex flex-col gap-3">
				{isLoading ? (
					...
					<>
						<Progress value={(votedCount ? votedCount / totalProjects : 0) * 100} />
						<p className="text-sm text-muted-foreground">You&apos;ve scored {votedCount} out of {totalProjects} projects</p>
					</>
				)}
			</CardFooter>
		</Card >
